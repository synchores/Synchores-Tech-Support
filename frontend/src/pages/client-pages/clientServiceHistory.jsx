...
export default function ClientServiceHistory() {
  ...
  const getStatusStyle = (status) => {
    ...
    return statusMap[status] ?? { bg: 'rgba(107, 114, 128, 0.15)', color: '#9ca3af' };
  };
  ...
  return (
    <div className="min-h-screen pt-24 pb-16" style={{ background: `linear-gradient(135deg, ${colors.blue900} 0%, ${colors.blue800} 100%)` }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {/* Service Timeline */}
        {loading && (
          <div className="p-6 rounded-xl" style={{ background: 'rgba(20, 40, 70, 0.35)', border: '1px solid rgba(107, 114, 128, 0.15)' }}>
            <p style={{ color: colors.textSecondary }}>Loading order history...</p>
          </div>
        )}

        {!loading && error && (
          <div className="p-6 rounded-xl" style={{ background: 'rgba(239, 68, 68, 0.08)', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
            <p style={{ color: '#fca5a5' }}>Failed to load history. Please try again.</p>
          </div>
        )}

        {!loading && !error && orders.length === 0 && (
          <div className="p-6 rounded-xl" style={{ background: 'rgba(20, 40, 70, 0.35)', border: '1px solid rgba(107, 114, 128, 0.15)' }}>
            <p style={{ color: colors.textSecondary }}>No orders yet. Place an order from the shop to start your history.</p>
          </div>
        )}
                ...
                <div className="flex gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <h3 className="text-white font-bold text-base">Order #{order.orderId}</h3>
                      <span
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <p className="text-xs font-semibold mb-1" style={{ color: colors.textMuted }}>Created</p>
                        <p className="text-white text-sm">{new Date(order.createdAt).toLocaleDateString()}</p>
                      </div>

                      <div>
                        <p className="text-xs font-semibold mb-1" style={{ color: colors.textMuted }}>Invoice</p>
                        <p className="text-white text-sm">{invoice?.invoiceNumber ?? 'Pending'}</p>
                      </div>

                      <div>
                        <p className="text-xs font-semibold mb-1" style={{ color: colors.textMuted }}>Total</p>
                        <p className="text-white text-sm font-bold">${Number(order.totalPrice).toFixed(2)}</p>
                      </div>
                    </div>
